...
function Index({ branches: data }) {
    ...
    useEffect(() => {
        const search = (query) => {
            if (!query) return setBranches(data);
            return setBranches(
                [...data].filter((branch) =>
                    Object.keys(branch).some((key) =>
                        key === 'name' || key === 'address' ?
                            branch[key].toString().toLowerCase().includes(query.toLowerCase())
                            : false
                    )
                )
            );
                            ...
                            <Table.Row key={ k } className="bg-white dark:border-gray-700 dark:bg-gray-800">
                                ...
                                <Table.Cell className='flex justify-center gap-x-2 -ml-6'>
                                    ...
                                    <Link
                                        ...
                                        href={ route('branches.destroy', branch.id) } method='delete' onSuccess={ () => successDelete(branch.id) }
                                    >
                                        Delete
                                    </Link>
                                </Table.Cell>
